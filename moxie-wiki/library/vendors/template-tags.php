...
if ( ! function_exists( 'moxie_wiki_comment' ) ) :
	/**
 * Template for comments and pingbacks.
 *
 * Used as a callback by wp_list_comments() for displaying the comments.
 */
	function moxie_wiki_comment( $comment, $args, $depth ) {
...
endif; // ends check for moxie_wiki_comment()

if ( ! function_exists( 'moxie_wiki_posted_on' ) ) :
	/**
 * Prints HTML with meta information for the current post-date/time and author.
 */
	function moxie_wiki_posted_on() {
 ...
 */
function moxie_wiki_categorized_blog() {
	...
	if ( '1' != $all_the_cool_cats ) {
		// This blog has more than 1 category so moxie_wiki_categorized_blog should return true.
		return true;
	} else {
		// This blog has only 1 category so moxie_wiki_categorized_blog should return false.
		return false;
	}
}

/**
 * Flush out the transients used in moxie_wiki_categorized_blog.
 */
function moxie_wiki_category_transient_flusher() {
	// Like, beat it. Dig?
	delete_transient( 'all_the_cool_cats' );
}
add_action( 'edit_category', 'moxie_wiki_category_transient_flusher' );
add_action( 'save_post',     'moxie_wiki_category_transient_flusher' );
