 ...
 */
function moxie_wiki_customize_register( $wp_customize ) {
	$wp_customize->get_setting( 'blogname' )->transport         = 'postMessage';
	$wp_customize->get_setting( 'blogdescription' )->transport  = 'postMessage';
	$wp_customize->get_setting( 'header_textcolor' )->transport = 'postMessage';
}
add_action( 'customize_register', 'moxie_wiki_customize_register' );

/**
 * Customizer Sanitization Functions
 */
function moxie_wiki_sanitize_text( $input ) {
	return wp_kses_post( force_balance_tags( $input ) );
}

function moxie_wiki_sanitize_checkbox( $input ) {
	if ( 1 == $input ) {
		return 1;
	} else {
		return '';
...
/**
 * Customizer Moxie Wiki Additions
 */

function moxie_wiki_add_customizer_theme_options($wp_customize) {
	...
	*/
	...
	$wp_customize->add_setting( 'moxie_wiki_add_link_color', array(
		'default'			=> '#000000',
		'sanitize_callback' 	=> 'maybe_hash_hex_color',
	) );

	$wp_customize->add_control( new WP_Customize_Color_Control(
		$wp_customize, 'moxie_wiki_add_link_color', array(
			'label'			=> __( 'Body Link Color', 'some-like-it-neat' ),
			'section'		=> 'colors',
			'settings'		=> 'moxie_wiki_add_link_color',
			'priority'		=> 6,
		)
	) );
	...
	*/
	$wp_customize->add_panel( 'navigation_panel', array(
		...
		$wp_customize->add_setting(
			'moxie_wiki_mobile_nav_label',
			array(
			'default'			=> 'Menu',
			'sanitize_callback' => 'moxie_wiki_sanitize_text',
			)
		);
		...
		// Mobile Settings
		$wp_customize->add_setting(
			'moxie_wiki_mobile_hide_arrow',
			array(
			'default'		=> 'No',
			'sanitize_callback'	=> 'moxie_wiki_sanitize_checkbox',
			)
		);
		...
		*/
		...
		$wp_customize->add_section( 'moxie_wiki_footer_section_settings', array(
			'priority' => 10,
			'capability' => 'edit_theme_options',
			'theme_supports' => '',
		...
		$wp_customize->add_control(
			'moxie_wiki_footer_left',
			array(
			'section'	=> 'moxie_wiki_footer_section_settings',
		...
		$wp_customize->add_setting(
			'moxie_wiki_footer_colophon',
			array(
			'default'			=> 'Moxie Wiki, by Moxie NYC - http://getmoxied.net',
			'sanitize_callback'	=> 'moxie_wiki_sanitize_text',
			)
		);
		$wp_customize->add_control(
...
add_action( 'customize_register', 'moxie_wiki_add_customizer_theme_options' );

/**
 * Binds JS handlers to make Theme Customizer preview reload changes asynchronously.
 */

function moxie_wiki_customize_preview_js() {
	wp_enqueue_script( 'moxie_wiki_customizer', get_stylesheet_directory_uri() . '/library/vendors/customizer/js/customizer.js', array( 'customize-preview' ), '20130508', true );
}
add_action( 'customize_preview_init', 'moxie_wiki_customize_preview_js' );
